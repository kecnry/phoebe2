...
def _inv_sin(theta, eps=1e-12):
    ...
    The 1/sin(theta) factors in the tangential displacement are a coordinate
    singularity of the spherical-harmonic basis, not a physical one. A mesh
    vertex sitting exactly on the pole otherwise produces 0/0 -> nan, which
    then propagates into the coordinates and poisons the whole light curve.
    """
    s = np.sin(theta)
